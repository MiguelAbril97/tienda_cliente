...
def productos_listar_api(request):
    ...
    response = requests.get('https://mabrala.pythonanywhere.com/api/v1/productos',headers=headers)
...
def productos_listar_mejorado_api(request):
    ...
    headers = {'Authorization':'Bearer '+env('ADMIN')}
    response = requests.get('https://mabrala.pythonanywhere.com/api/v1/productos-mejorado',headers=headers)
    productos = response.json()
...
def calzado_listar(request):
    ...
    response = requests.get('https://mabrala.pythonanywhere.com/api/v1/calzados',headers=headers)
...
def consolas_listar(request):
    ...
    response = requests.get('https://mabrala.pythonanywhere.com/api/v1/consolas',headers=headers)
